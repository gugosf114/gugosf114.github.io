...
`order-packaging-film.mjs` controls a silent sixteen-second H.264 MP4 in the opening preview. It is pre-rendered from the 3D scene at 1080×1080 / 30 fps: cookie close-up, clear sleeve, twelve-cookie window box, ribbon, and full turn. Visitors do not need WebGL. The introduction now attempts muted inline autoplay on both desktop and mobile, with no Play, Replay, Skip, or timeline controls in the order flow. It runs once and holds the final frame. Entering the editor stops playback and detaches the video source; returning to Start does not replay it. Hidden/offscreen playback pauses and resumes when visible. If browser autoplay policy rejects playback, the poster remains and an ordinary page interaction can retry playback; a media-load failure leaves the static cookie and ordering controls usable. The standalone player under `media/packaging-preview.html` retains native video controls.
...
Tests: `node --test test/order-packaging-film.test.mjs` covers twelve slots and the finite full turn. `node test/order-packaging-film.browser.cjs` verifies native decoding, uninterrupted silent autoplay, absent playback controls, the held final frame, mobile autoplay, a failed-media fallback, and editor handoff. `node test/order-packaging-lifecycle.browser.cjs` checks editor-driven cancellation while media loads and pause/resume in background tabs. The local media server must support HTTP byte-range requests for seek tests.

### Design and ordering

The opening screen offers **Choose a design**, **Make my own**, and **Help me find the words (AI, optional)**. There are 110 authored designs: 15 each for Birthday, Anniversary, Thank You, Congratulations, Just Because, and Edgy, plus 20 Corporate company-logo designs. Corporate includes interns, onboarding, policies, launches, project completion, work anniversaries, promotions, retirement, hackathons, offsites, partnerships, quarter close, and holidays. Eight tech backgrounds and a brand-accent picker support logo uploads; corporate designs reserve a white logo panel, fit the complete logo above the message, and require a logo before approval. Personality filters are Sweet, Funny, Bold, Romantic, Edgy, and Professional; designs can carry multiple personality tags. Edgy is labeled as grown-up humor. Gallery previews render as they approach the viewport. Customers can use a design as shown, personalize it, or start with their own photo.

`order-studio-designs.mjs` defines the template catalog, 26 preset backgrounds plus a custom-color option (including rainbow and trans Pride flags, retro checks, disco, cosmic, leopard, comic, garden, sunset, electric, money, office politics, tax-season, campaign, red-tape, main-character, and zero-filter themes), local pattern drawing, and editable text rendering. Background, optional photo, and words are separate layers. The fifteen newer backgrounds include optional editable one-liners; selecting a background never replaces the customer’s words unless they press Use this line. `order-studio-compose.mjs` owns their controls, including customer background uploads, lettering, colors, size, position, and optional names/dates. Text-only designs export a PNG source along with the clean print artwork and approved cookie preview, so the existing private order service receives the same three-file contract without requiring a customer photo.

Optional AI currently assists with **message writing**, using the existing bakery chatbot service. Only the brief and selected occasion, tone, and age group are sent after the customer presses Generate. Manual design and upload paths make no AI request. The three suggestions stay editable. AI-generated cartoon/realistic artwork is not connected in this version; the existing cake-preview endpoint is not treated as a verified flat-art generator.

Additional tests: `node test/order-studio-compose.browser.cjs` covers templates, filters, editable text, background uploads, a text-only order through mocked payment, preserving layers when photos change, and optional AI-to-editor behavior. The test intercepts AI and payment requests. The live AI service was separately verified with a synthetic gardening-birthday brief and no customer images.

The photo route in `buy-now.html` remains a single guide: upload → shape and framing → background → review → delivery → payment. Keep the whole photo goes directly to review. Remove the background asks the buyer to tap what to keep on the framed cookie, then offers Looks good or Clean up a spot (an explicit tap-to-remove mode). Undo restores each previous cut. Zoom and dragging remain available on the cut result and at review; manual brush correction is optional. The photo route uses the customer's upload throughout, without a slideshow. `order-studio.css` owns its isolated layout and responsive styles; sitewide navigation and marketing styles do not load here.

`order-studio.mjs` owns navigation, browser history, image edits, multi-photo orders, approval, and lazy payment loading. `order-studio-art.mjs` uses one normalized crop transform for the live cookie, source-coordinate tap mapping, and saved print artwork. `order-studio-cutout.mjs` loads the pinned MediaPipe MagicTouch model only when a customer selects a subject. Cleanup adds negative taps while retaining the original positive anchor. It can only remove pixels from the current mask; Undo can restore them. Cutouts keep the original source dimensions and never auto-center or reset the user's crop. The editor downsizes its working copy to a maximum 1600-pixel edge and retains the original file for the bakery.

Background-flow checks: `node test/order-studio-cutout.browser.cjs` with the same browser/URL environment variables as the main walkthrough. The default uses a controlled mask for repeatable failure, cleanup, and exact-undo checks. Set `REAL_CUT=1` to exercise the actual MediaPipe download, positive selection, negative cleanup, and saved artwork. Both modes operate locally on the image and stop before payment.
...
AI cookie design studio: “Help me create a design” now creates three original backgrounds with matching editable messages. The dedicated Google Cloud function in cloud-functions/cookie-design uses Claude Sonnet 5 and Gemini 3 Pro Image with the bakers-agent Secret Manager credentials; no credentials are included in the site. Customers can choose visual styles, add their own photo locally, and regenerate only the background.
