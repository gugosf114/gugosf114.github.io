...
`buy-now.html` is a single guided photo-cookie studio: upload → shape and framing → background → review → delivery → payment. Keep the whole photo goes directly to review. Remove the background asks the buyer to tap what to keep on the framed cookie, then offers Looks good or Clean up a spot (an explicit tap-to-remove mode). Undo restores each previous cut. Zoom and dragging remain available on the cut result and at review; manual brush correction is optional. It uses the customer's upload throughout, with no gallery or slideshow. `order-studio.css` owns its isolated layout and responsive styles; sitewide navigation and marketing styles do not load here.
